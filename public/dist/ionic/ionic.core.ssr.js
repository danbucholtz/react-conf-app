...
var s=document.querySelector("script[data-namespace='ionic']");if(s){publicPath=s.getAttribute('data-path');}
(function(publicPath){
    /** Ionic global **/
...
(function(window, document, Context, appNamespace, publicPath) {
  ...
  function defineMember(plt, cmpMeta, elm, instance, memberName) {
    ...
    if (1 === memberType || 5 === memberType || 2 === memberType) {
      ...
      4 === memberType && // @Prop({ connect: 'ion-loading-ctrl' })
  ...
  function setValue(plt, elm, memberName, newVal) {
    ...
    if (newVal !== oldVal) {
      ...
      internalValues[PROP_WILL_CHG + memberName] && // this instance is watching for when this property WILL change
  ...
  function render(plt, elm, cmpMeta, isUpdateRender) {
    ...
    if (instance.render || instance.hostData || hostMeta) {
      ...
      hostMeta && (// component meta data has a "theme"
  ...
  function renderUpdate(plt, elm, isInitialLoad) {
    ...
    try {
      ...
      elm.$initLoad() : // fire off the user's componentDidUpdate method (if one was provided)
